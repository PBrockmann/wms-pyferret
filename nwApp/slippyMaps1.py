...
<div class="selectInput"><select id="file" style="width: 100%;"></select></div>
</div>

<div class="forSelect">
<label for="command">Command to run:</label>
<div class="selectInput"><select id="command" style="width: 100%;"></select></div>
</div>

<div class="forSelect">
<label for="variable">Variable to display:</label>
<div class="selectInput"><select id="variable" style="width: 100%;"></select></div>
</div>
...
$('#file').select2({
	tags: true,
        data: [ {id: 0, text: 'levitus_climatology'}, 
		{id: 1, text: 'monthly_navy_winds'}
	]
});

$('#command').select2({
	tags: true,
        data: [ {id: 0, text: 'shade/x=-180:180/y=-90:90/lev=10v/pal=mpl_PSU_inferno'},
                {id: 1, text: 'shade/x=-180:180/y=-90:90/lev=20/pal=default'}
	]
});

$('#variable').select2({
	tags: true,
        data: [ {id: 0, text: 'temp[k=@max]'}, 
		{id: 1, text: 'salt[k=@max]'},
		{id: 2, text: 'uwnd[l=1]'},
		{id: 3, text: 'vwnd[l=@ave]'},
	]
});
...
$("#addMap").on('click', function() {
	file = $('#file').select2('data')[0].text;
	command = $('#command').select2('data')[0].text;
